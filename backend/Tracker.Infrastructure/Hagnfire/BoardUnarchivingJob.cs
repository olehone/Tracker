...
internal class BoardUnarchivingJob(IUnitOfWorkFactory unitOfWorkFactory,
    ServiceBusClient serviceBus,
    IOptions<ServiceBusOptions> serviceBusOptions) : IBoardUnarchivingJob
{
    public async Task ExecuteAsync()
    {
        await using var uow = unitOfWorkFactory.Create();

        var boards = await uow.BoardRepository.GetByArchiveStatusAsync(ArchiveStatus.PendingUnarchive);
        ...
        var queue = serviceBusOptions.Value.BoardUnarchiveQueueName;
        var subject = serviceBusOptions.Value.BoardUnarchiveSubjectName;
        ...
        ServiceBusMessageBatch batch)
    {
        ...
        {
            var payload = new UnarchiveBoardMessage(board.Id);
